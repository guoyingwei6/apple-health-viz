...
| 模块 | 内容 |
|------|------|
| ❤️ 心率 | 静息心率趋势、HRV 趋势、心率分布直方图、心率恢复、步行平均心率 |
| 😴 睡眠 | 每日睡眠时长、月均趋势、短睡天数统计、睡眠改善建议 |
| 🚶 活动量 | 月均步数、90天热力图、活动卡路里、站立时间、步行/跑步距离、爬楼层数 |
| 💪 体能 | VO₂Max 趋势与评级、6分钟步行测试、步行稳定性 |
| ⚖️ 体重 | 体重趋势、瘦体重趋势、BMI 折线图、体脂率（男女不同参考区间） |
| 🧬 进阶指标 | 呼吸频率、血氧、血压、体温、跑步/骑行/游泳专项指标 |
| 📊 综合评分 | 6个维度三色评级（🟢🟡🔴）、健康提示、风险说明 |
| 🩺 健康建议 | 每类页面基于纯脚本规则判断稳定性、是否达标、改善方向和就医提示 |
| 📤 数据导出 | 导出清洗后的 JSON / CSV，包含 summary、daily、monthly 和评级结果 |
...
4. 在「选择数据来源」页确认要纳入分析的来源，默认选择 Apple Watch / iPhone
5. 进入仪表盘，点击顶部 Tab 切换各模块
6. 可点击右上角导出 JSON / CSV，用于备份、Excel 或进一步分析
...
Apple Health 导出文件可能同时包含 Apple Watch、iPhone 和第三方 App 写入的同类数据。为减少重复计数，上传解析后会先展示所有 `sourceName` 来源：

- 默认选择名称包含 Apple Watch / iPhone 的来源
- 可手动勾选或取消第三方 App
- 睡眠会基于已选来源重新聚合
- 导出的 JSON / CSV 也只包含已选来源后的清洗结果

### 健康建议

每个分类页顶部都有一张「健康建议」卡片，使用本地脚本规则生成，不调用 AI 或外部 API。建议内容包括：
...
> 建议仅基于 Apple Health / Apple Watch 数据和通用阈值规则，不构成医疗诊断。
...
| 指标 | 触发提示 |
|------|----------|
| 静息心率 | 多日高于 90 bpm、低于 45 bpm，或伴随胸闷、心悸、头晕 |
| HRV | 近期明显偏低，提示压力、疲劳或恢复不足 |
| 睡眠 | 连续多周平均睡眠低于 5h，或短睡占比过高 |
| 6分钟步行 | 距离偏低且伴随气短、胸闷或下肢无力 |
| BMI | ≥30 或 <17 |
| 血压 | 多次高于 140/90 mmHg |
| 血氧 | 多次低于 92% |
...
- **来源过滤：** 基于 Apple Health `sourceName` 本地筛选
- **健康建议：** 纯脚本阈值和趋势规则，无需 AI/API
- **导出：** 清洗后的 JSON / CSV
- **测试：** Vitest
...
- **来源过滤：** 所有图表、评级、建议和导出均基于用户选择的数据来源。
- **VO₂Max 评级：** 仅在填写性别和年龄后显示，参考 American College of Sports Medicine 标准。
- **稳定性判断：** 对比最近一段数据与上一段数据，变化较小时视为稳定；数据不足时提示继续积累。
- **导出内容：** JSON / CSV 包含清洗后的 daily、monthly、summary 和 ratings，不导出完整原始 XML。
