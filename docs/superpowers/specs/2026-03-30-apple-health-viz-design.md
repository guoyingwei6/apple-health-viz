...
上传页面（或仪表盘顶部）提供可选填写的性别和年龄字段：

- **字段：** 性别（男 / 女）、年龄（数字输入）
- **用途提示文案：** "仅用于 VO₂Max 有氧能力评级，不上传、不存储"
- **不填则：** VO₂Max 模块只展示趋势折线图，不显示三色评级和文字评价
- **存储：** 仅保留在内存中（React state），页面刷新后清空，不写入 localStorage
...
```js
{
  ...
  // 排除 InBed；按"当日中午12点前结束的睡眠归属前一天"规则聚合；
  // 每天只取最长连续一段（排除午睡：持续时长 < 3h 且开始时间在 09:00-20:00 之间的记录视为午睡，不计入）
...
| VO2Max（男） | 年龄18-29:≥44 / 30-39:≥42 / 40-49:≥39 / 50+:≥36 | 低于🟢阈值1-6 | 低于🟢阈值>6 |（需填写性别年龄，否则不评级）|
| VO2Max（女） | 年龄18-29:≥40 / 30-39:≥38 / 40-49:≥35 / 50+:≥32 | 低于🟢阈值1-6 | 低于🟢阈值>6 |（需填写性别年龄，否则不评级）|
